...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    const lowercase_answer = trimmed_answer.toLowerCase();
    return question.expected.toLowerCase() === lowercase_answer;
}
...
export function toShortForm(question: Question): string {
    const first_ten = question.name.substring(0, 10);
    return question.id.toString() + ": " + first_ten;
}
...
export function toMarkdown(question: Question): string {
    let return_value: string = "# " + question.name + "\n" + question.body;
    let newString = "";
    if (question.type === "multiple_choice_question") {
        return_value += "\n";
        //add dashes and space to each string
        const add_dashes: string[] = question.options.map(
            (str: string): string => "- " + str
        );
        //put array into a string
        const middle = add_dashes.reduce(
            (currentString: string, str: string) => currentString + str + "\n",
            ""
        );
        const tempString = return_value + middle;
        newString = tempString.slice(0, -1);
    } else {
        newString = return_value;
    }
    return newString;
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const newName = "Copy of " + JSON.parse(JSON.stringify(oldQuestion.name));
    const newQuestion = {
        ...oldQuestion,
        name: newName,
        published: false,
